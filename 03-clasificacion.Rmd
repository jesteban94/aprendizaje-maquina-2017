...
Una población de mujeres quienes tenían al menos 21 años, de herencia Pima India y viven cerca a Phoenix Arizona, fueron examinadas para diabetes de acuerdo con los criterios de la Organización Mundial de la Salud. Los datos fueron recogidos por el Instituto nacional de diabetes y enfermedades digestivas y renales US. Usamos los 532 registros completos después de descartar los datos (principalmente los faltantes) en suero de insulina.

- npreg número de embarazos.
- glu concentración de plasma de glucosa en un test oral de tolerancia de glucosa.
- bp presión arterial diastólica (mm Hg).
- skin triceps espesor del pliegue de piel (mm).
- bmi índice de masa corporal (peso en Kg/ (altura en m^2)).
- ped función del árbol genealógico de diabetes.
- age edad en años.
- Tipos si o no, para diabetes de acuerdo al criterio WHO.
...
mediante
$$\hat{E}_{rr} = \frac{1}{m} \sum_{j=1}^m I(\hat{G}(x_0^{(j)}) \neq g_0^{(j)}),$$
